...
const Ventas = () => {
	...
	const init = async () => {
		const { data } = await ApiRequest().get('/listarven')
		setVentasList(data)
	}
	...
	const columns = [
		...
		{ field: 'referencia', headerName: 'Referencia de equipo', width: 180 },
		{ field: 'cantidad', headerName: 'Cantidad', width: 120 },
		{ field: 'fecha', headerName: 'Fecha de Venta', width: 120 },
	...
	const onDelete = async (id) => {
		try {
			const { data } = await ApiRequest().post('/eliminarven', { id: id })
			setMensaje({
	...
	const onSubmit = async () => {
		try {
			const { data } = await ApiRequest().post('/guardarven', body)
			handleDialog()
	...
	const onEdit = async () => {
		try {
			const { data } = await ApiRequest().post('/editarven', body)
			handleDialog()
			setBody(initialState)
			setMensaje({
				ident: new Date().getTime(),
			...
			<Page title="Inventario">
				...
				<Container maxWidth='lg'>
					<Box sx={{ pb: 5 }}>
						...
						<CSVLink data={ventasList} filename={"ventas.csv"}><Button startIcon={<AddOutlined />} variant='contained' color='primary'>Exportar a CSV</Button></CSVLink>
					</Box>
					<Grid container spacing={2}>
						...
						<Grid item xs={12} sm={12}>
							<CommonTable data={ventasList} columns={columns} />
						</Grid>
					</Grid>
				</Container>
			</Page>
